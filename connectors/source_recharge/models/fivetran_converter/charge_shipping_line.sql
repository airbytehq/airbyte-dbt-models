...
(
    select
        ...
        m.value:price::string as price,  -- Snowflake: Extract JSON keys using colon notation and cast to appropriate types
        m.value:code::string as code,
        m.value:title::string as title
    FROM 
    {{ source('source_recharge', 'charges') }},
    lateral flatten(input => parse_json(shipping_lines)) m  -- Snowflake: Use LATERAL FLATTEN to unnest JSON array
...
(
    select
        ...
        m.value ->> 'price' as price,  -- Postgres: Extract JSON key as text using ->>
